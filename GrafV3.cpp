...
void GrafPonderat::Kruskall()
{
	...
	{
		int indexIndici = i - 1;
		if (KruskallFindMultime(multime, listaMuchiiPonderate[indici[indexIndici]].first.first) != KruskallFindMultime(multime, listaMuchiiPonderate[indici[indexIndici]].first.second))
		{
			suma += listaMuchiiPonderate[indici[indexIndici]].second;
			KruskallReuniune(multime, listaMuchiiPonderate[indici[indexIndici]].first.first, listaMuchiiPonderate[indici[indexIndici]].first.second);
...
int main()
{
	...
	G.setMatricePonderata();

	G.printMatriceCosturiMinime();
